...
function configure_make() {
    ...
    PREFIX_DIR="${pwd_path}/../output/ios/libevent-${ARCH}"
    if [ -d "${PREFIX_DIR}" ]; then
        rm -fr "${PREFIX_DIR}"
    fi
    mkdir -p "${PREFIX_DIR}"

    OUTPUT_ROOT=${TOOLS_ROOT}/../output/ios/libevent-${ARCH}
    mkdir -p ${OUTPUT_ROOT}/log

    set_ios_cpu_feature "libevent" "${ARCH}" "${IOS_MIN_TARGET}" "${SDK}"
...
function lipo_library() {
    LIB_SRC=$1
    LIB_DST=$2
    LIB_PATHS=("${ARCHS[@]/#/${pwd_path}/../output/ios/libevent-}")
...
cp -r "${TOOLS_ROOT}/../output/ios/libevent-${ARCHS[0]}/include/"* "${LIB_DEST_DIR}/include"
